export const page = {
  ...
  selectPhonesOnPage(phone1, phone2, phone3, selectOption, useSelect) {
    ...
    cy.visit('#/phones').then(() => {
      if (useSelect) {
        cy.get('select').first().select(selectOption);
  ...
  addToFavorites() {
    ...
    cy.getByDataCy('addToFavorite').click();
  ...
  select(index, value) {
    cy.get('select').eq(index).select(value);
  },

  assertValue(assert, value) {
    cy.getByDataCy('productList').children().should(assert, value);
  },
};
