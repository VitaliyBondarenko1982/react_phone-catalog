...
describe('Product details', () => {
  it('should have "ProductCard" as a link to the details page', () => {
    ...
    cy.url().should('contain', 'motorola-xoom-with-wi-fi');
  });

  it('shuld show product description on the page', () => {
    cy.intercept('**products.json', { fixture: 'oneProduct' });

    cy.fixture('productDetails.json').as('prodDetails');

    cy.visit('/');

    cy.get('@prodDetails').then((prodDetails) => {
      cy.get('a[href*="motorola"]').click();

      cy.getByDataCy('productDescription').should(
        'contain',
  ...
  it('should have "Back" button implemented', () => {
    ...
    cy.getByDataCy('backButton').click();
  ...
  it('should have "Breadcrumbs" element implemented', () => {
    ...
    cy.getByDataCy('breadCrumbs').find('a[href="#/"]');

    cy.getByDataCy('breadCrumbs').find('a[href="#/tablets"]');
  });
});
